...
app.config["SQLALCHEMY_DATABASE_URI"]="mysql://root:@localhost/coding1"
db.init_app(app)
...
class RegistrationForm(FlaskForm):
    ...
    password=StringField(label="Password",validators=[DataRequired(),Length(min=4,max=20)])
...
def logout():
    logout_user()
    flash("you have been logged out successfuly ! ","info")
    return redirect(url_for("login"))
...
def register():
    ...
    if request.method=="POST" and form.validate_on_submit():
        ...
        db.session.commit()
        flash("Account Created Successfully!<br> you an nov log in.","success")
        return redirect(url_for("login"))
